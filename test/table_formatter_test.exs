defmodule TableFormatterTest do
  ...
  test "split_into_columns" do
    cs = split_with_three_columns()
    assert length(cs) == length(@headers)
    assert List.first(cs) == ["r1 c1", "r2 c1", "r3 c1", "r4 c1"]
    assert List.last(cs) == ["r1+++c4", "r2 c4", "r3 c4", "r4 c4"]
  end
  ...
  test "output is correct" do
    ...
    # formatter gets rid of trailing spaces, so can't test as-is
    assert result != """
           c1    | c2     | c4
           ------+--------+--------
           r1 c1 | r1 r2  | r1+++c4
